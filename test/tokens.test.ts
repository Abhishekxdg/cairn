...
describe("token comparison (the headline stat)", () => {
  it("reports ratio and savings of Cairn vs a baseline scan", () => {
    const coldScan = "x".repeat(40000); // ~10k tokens of repo dump
    const cairn = "x".repeat(2000); // ~500 tokens of CONTEXT.md
    const cmp = compareTokens(coldScan, cairn);
    expect(cmp.baselineTokens).toBeGreaterThan(cmp.cairnTokens);
    expect(cmp.ratio).toBeGreaterThan(1);
    expect(cmp.saved).toBe(cmp.baselineTokens - cmp.cairnTokens);
  });

  it("never divides by zero", () => {
    const cmp = compareTokens("some baseline", "");
    expect(cmp.ratio).toBe(1);
    expect(cmp.cairnTokens).toBe(0);
  });
});
