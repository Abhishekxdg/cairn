#!/usr/bin/env node
// 10M-scale benchmark for AJP. Not part of the test run (it takes a while).
//
...
const dir = mkdtempSync(join(tmpdir(), "ajp-bench-"));
const dbPath = join(dir, "journal.db");
const mb = (n) => (n / 1e6).toFixed(1);

console.log(`AJP benchmark — ${COUNT.toLocaleString()} events\n`);
