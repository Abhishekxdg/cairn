...
AJP is layered so that every higher layer is a pure projection of the one below.
...
| `cli/index.ts` | `ajp` command-line |
| `mcp/server.ts` | MCP tools + resources |

## Data flow

1. An agent appends an event (`appendEvent` / `ajp append` / `append_event`
...
2. After an append, the SDK checks the snapshot policy and, if due, materializes
   a snapshot.
3. A read (`getState` / `query_context` / `ajp timeline`) loads the latest
...
- Schema drift → `ajp migrate` brings an old journal forward.
