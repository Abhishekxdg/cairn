...
| What it did | `ajp` missing → "scanning docs/tests to infer", explored 6 files | "Using AJP project memory. Reading current context" |
| Orientation result | **WRONG** — reconstructed the *Stated v0.2* work from the dirty tree (not this project) | **CORRECT** — "Ship AJP v0.1", the SQLite-WAL decision *with its reason*, the next step |
