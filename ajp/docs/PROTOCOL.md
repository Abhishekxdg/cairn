# Agent Journal Protocol — Specification (v1)

Status: draft · Schema version: 1

This document specifies the on-disk format, the event shape, and the derivation
rules that make AJP a protocol rather than a library. Any implementation in any
...
against the JSON export (`ajp export`).
