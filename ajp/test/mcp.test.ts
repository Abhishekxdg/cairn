...
beforeEach(() => {
  prev = process.env["AJP_ROOT"];
});
afterEach(() => {
  if (prev === undefined) delete process.env["AJP_ROOT"];
  else process.env["AJP_ROOT"] = prev;
});

async function connect() {
  const dir = tempDir();
  init(dir, { name: "MCP Test" });
  process.env["AJP_ROOT"] = dir;
...
describe("AJP MCP server", () => {
  ...
  it("serves the state resource", async () => {
    const { client } = await connect();
    const read = await client.readResource({ uri: "ajp://state" });
