#!/usr/bin/env node
/**
 * Product eval — does the Cairn memory layer actually help an agent, vs working
 ...
 *   - cairn       WITH CAIRN: derived .agent/CONTEXT.md recall
 *
...
// Returns { events } to feed Cairn, and a parallel ground-truth of final state.
// ---------------------------------------------------------------------------
...
// WITH CAIRN: derived, always-current recall.
function cairnArtifact(events) {
  const dir = mkdtempSync(join(tmpdir(), "cairn-eval-"));
...
function score(artifact, truth, kind) {
  ...
  if (kind === "cairn") checks.currentTask = artifact.includes("Current:") && has(truth.currentTask);
  ...
  if (kind === "cairn") checks.liveDecision = has(truth.liveDecision);
  ...
  checks.ownership = kind === "cairn" ? artifact.includes(truth.currentOwner)
    : kind === "naive" ? (has(`owner ${truth.currentOwner}`) && count(/owner /g) <= 1)
    : false;

  // explicit next step
  checks.nextStep = kind === "cairn" ? /Next:|recommend/i.test(artifact) : false;
...
// ===========================================================================
console.log("Cairn product eval — with / without / alternatives\n");
...
// --- Eval 1+2: orientation cost & completeness at a realistic 60 sessions ---
{
  ...
  const rows = [
    ...
    ["WITH Cairn recall", cairnArtifact(events), "cairn"],
  ];
  ...
  const aj = TOK(cairnArtifact(events)), bl = TOK(baselineArtifact(events)), nv = TOK(naiveArtifact(events));
  console.log(`\n  → Cairn recall is ${(bl / aj).toFixed(1)}× cheaper than re-reading the repo, ${(nv / aj).toFixed(1)}× cheaper than the accreted handoff — and the only one that answers all 5.`);
}

// --- Eval 3: scale curve — does the cost stay flat as history grows? --------
{
  console.log("\nEVAL 3 — scale: orientation ~tokens vs project history\n");
  console.log(`  ${pad("sessions", 10)}${padL("baseline", 12)}${padL("naive-md", 12)}${padL("cairn", 8)}`);
  console.log("  " + "-".repeat(42));
  for (const n of [10, 50, 200, 1000]) {
    const { events } = makeHistory(n);
    console.log(`  ${pad(n, 10)}${padL(TOK(baselineArtifact(events)), 12)}${padL(TOK(naiveArtifact(events)), 12)}${padL(TOK(cairnArtifact(events)), 8)}`);
  }
  console.log("\n  → naive handoff grows unbounded; Cairn recall stays ~flat (it's a derived summary, not an accretion).");
}
...

{
  ...
  const dir = mkdtempSync(join(tmpdir(), "cairn-eval-conc-"));
  ...
  // WITH CAIRN: SQLite WAL serializes writers, idempotent on id
  ...
  console.log(`  ${pad("WITH Cairn (SQLite WAL)", 26)}${padL(want, 10)}${padL(cairnKept, 8)}${padL(lost(cairnKept), 24)}`);
  console.log("\n  → unlocked file memory silently loses concurrent updates; Cairn loses none.");
  rmSync(dir, { recursive: true, force: true });
}
