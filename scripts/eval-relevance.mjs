 ...
 *   cairn        BM25 · fileIDF · recency · path-prior   (the system)
...
function rankCairn(corpus, query, now) {
...
function backtest(corpusAll, truth, now) {
  const rankers = {
    cairn: rankCairn, "no-IDF": rankNoIdf, "path-only": rankPathOnly, recency: rankRecency, random: rankRandom,
  };
  const agg = {};
...
// ===========================================================================
console.log("Cairn relevance eval — task → relevant files (leave-one-out backtest)\n");
...
// --- Synthetic track --------------------------------------------------------
{
  ...
  const aj = agg.cairn, noidf = agg["no-IDF"];
...
// history corpus empty.
{
  ...
  const dir = mkdtempSync(join(tmpdir(), "cairn-cold-"));
  ...
  const variants = {
    ...
    "cairn(index)": (q) => rankFiles(null, q, { corpus: [], graph, now: 0, k: 1000 }).map((f) => f.path),
...
// --- Real track -------------------------------------------------------------
{
  ...
  if (!existsSync(dbPath)) {
    console.log("  (no .agent/journal.db — run `cairn sync` first)");
  } else {
