...
A journal written today must still open in a decade. Cairn versions the schema and
...
```bash
cairn migrate     # → "Migrated schema v0 → v1" or "Already at schema v1"
```
...
export (`cairn export`) is the stable interchange format across versions.
