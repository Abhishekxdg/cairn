...
export function migrate(db: Database): number {
  ...
  // A journal written by a NEWER cairn than this one can have columns/tables this
  // binary doesn't understand. Forward migrations can't undo that, so fail loud
  // rather than read it wrong and risk silent data loss.
  if (from > latest) {
    throw new Error(
      `journal schema v${from} is newer than this cairn (understands v${latest}). ` +
        `Upgrade cairn to open it.`,
    );
  }
